...
if (isset($_REQUEST["submit"])) {
    try{
        ...
        $stmt = null;
        $stmt = $con->prepare("INSERT INTO tbl_designation (dname, created_by, created_dt) 
...
if (isset($_REQUEST["update"])) {
    try{
       ...
       $updated_by = $_SESSION["user_id"]; 0;
        ...
        $stmt = null;
        $stmt = $con->prepare("UPDATE tbl_designation SET 
                        dname=:dname, 
...
if (isset($_REQUEST["did"])) {
    $stmt = null;
    $stmt = $con->query("UPDATE tbl_designation SET del_status=1 WHERE id=" . $converter->decode($_REQUEST["did"]));
...
if (isset($_REQUEST["id"])) {
    $rs = $con->query("SELECT * FROM tbl_designation where id=" . $converter->decode($_REQUEST["id"]));
    if ($rs->rowCount()) {
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="box box-primary">
					...
					<br />
                    <form id="thisForm" name="thisForm" action="designation.php" method="post">
                        <div class="box-body">
                            <div class="form-group">
                                <label class="control-label">Designation Name (In CAPS) <span class="err">*</span></label>
                                <input type="text" class="form-control" value="<?php echo $dname; ?>"name="txt_dname" id="txt_dname" placeholder="Enter The Designation Name" title="Enter The Designation Name" autocomplete="off" autofocus="autofocus" onKeyPress="return isCapitalWithSpace(event);" required oninvalid="this.setCustomValidity('Please enter the designation name...!')" oninput="this.setCustomValidity('')" />
                            </div>
                        </div>
                    ...
                    <div class="box-body">
                        <div class="dt-responsive table-responsive">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $rs = $con->query("SELECT * FROM tbl_designation where del_status=0 order by dname");
                                    if ($rs->rowCount()) {
                                        $sno=1;
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="designation.php?id=<?php echo $converter->encode($obj->id); ?>" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;
                                            <?php }
                                            if(in_array('deleteDepartment',$menu_permission)){
                                            ?>
                                            <a href="designation.php?did=<?php echo $converter->encode($obj->id); ?>" title="Delete" onclick="return confirm('Are You Sure Want To Delete?');"><i class="fa fa-trash-o" aria-hidden="true"></i>
                                            </a>
                                            <?php } ?>
                                        </td>
                                    </tr>
...
$(document).ready(function() {
    ...
    $('#masterDesignationSubNav').addClass('active');
});
</script>
